...
| ID | Review item | Status | Checked-in evidence | Required disposition / proof |
| --- | --- | --- | --- | --- |
| V-01 | Manifest-controlled constrained/standard/priority profiles | `SATISFIED` | The protected venue profile defines `CONSTRAINED_1080P30` at 2.5-3 Mbps, `STANDARD_1080P30` at 4-5 Mbps, and selected `PRIORITY_1080P60` at 8-12 Mbps, with an explicit enabled Camera 1-8 set, source/output assignments, and a separate operational `TIER_1`/`TIER_2`/`TIER_3` priority. | Use only profiles physically supported by each declared camera model. Never infer operational priority from output fps. |
