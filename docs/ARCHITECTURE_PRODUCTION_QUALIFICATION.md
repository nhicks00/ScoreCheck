...
| R-09 | Overlay exception does not interrupt program media | `SATISFIED` | The scorebug error boundary now retries only its own subtree once and then renders transparently. It never navigates or reloads the Program page, so a persistent overlay exception cannot restart the video, audio graph, or Egress scene. | Inject one bounded overlay render exception during the synthetic Program gate and retain media-continuity evidence. |
