...
| I-03 | Warm ingest replacement | `PARTIAL` | `ingest-recovery.mjs` implements the operator-confirmed transaction model. The DigitalOcean provider has an exact direct-reassignment primitive that verifies Reserved-IP ownership, unlocked state, active destination, common region/VPC, completed provider action, and final ownership. The lifecycle now retains and restores the stable ingest endpoint's complete host-owned Caddy TLS state, so replacement does not depend on fresh ACME issuance. No unassigned interval is introduced. | The remaining SSH/service adapter must stage the spare ingest role, apply its network policy, rebind and verify all compositors, switch monitoring, preserve durable transaction state, and restore the spare role. Implement and run it only in a protected 12-host rehearsal, then select dual-role spare or a thirteenth host from measured RTO. |
