...
| Keep WHEP only for low-bandwidth operator inspection | Rejected for ordinary inspection by physical evidence | Authorized public WHEP signaling succeeded from the current Mac network but media never arrived; the same physical Camera 3 session passed over the private compositor VPC. Chromium and iPhone-style WebKit both passed buffered HLS operator inspection. | Use buffered HLS for ordinary operator inspection and retain WHEP only for timing-sensitive commentary, scoring, and explicit path diagnostics. The overview remains reader-free. |
...
| Exact-once Egress recovery | Accepted for full worker-container loss on normal latency | The July 29 Camera 5 low-latency viewer stalled for 24.001 seconds. Camera 6 then showed that normal latency plus the two-second/two-observation supervisor reduced the stall to 1.499 seconds, but its scene was too dark for visual acceptance. The final clearly nonblack Camera 5 normal-latency gate recycled the complete Egress worker, admitted exactly one replacement, and limited the uninterrupted external-viewer stall to 752 ms with continuous audio and no evaluator problem. The stop path also now retains ownership and intent until LiveKit reports an empty active set. | Keep normal latency, the two-second/two-observation supervisor, full worker recycle, exact immutable ownership, and drain-before-release. Proceed to local-renderer/control-plane-loss recovery rather than repeating the accepted fault. |
...
| YouTube backup ingestion | Endpoint compatible; transparent handoff rejected | Commit `d3346358e` accepts YouTube's exact backup RTMPS `?backup=1` address and a real backup-only interval stayed provider active/good with the unlisted broadcast live. The uninterrupted viewer still stalled for 5.997 seconds and later moved backward 0.386 seconds when returning to primary. Both provider transitions remained green. | Keep backup support as operator-controlled emergency redundancy only. The current production recovery path remains normal-latency exact-owner single-output replay, which measured 752 ms. Do not add dual publishing to ordinary coverage without a later frame/timestamp-synchronized design that independently passes the two-second viewer gate. |
| Derived media authorization | Satisfied | Anonymous HLS/WHEP returned 401. Authorized HLS master, child playlist, initialization object, and media object succeeded with inherited event credentials. A private-VPC WHEP session received audio/video and deleted with HTTP 200. Sanitized runtime logs and monitor telemetry contained no exact read credential. | Keep the scoped hard cut. Ordinary operator detail inspection now uses the AAC `courtN_program` HLS rendition; timing-sensitive consumers retain WHEP. |
...
1. Exact-owner recovery on a clearly nonblack normal-latency physical source passed with a 752 ms maximum viewer stall, no pause/audio reset, moving-frame evidence, complete worker recycle, and no duplicate publisher.
...
5. Validate actual output conformance and persistent interruption-slate behavior against bounded physical feeds and unlisted destinations.
...
4. YouTube backup ingestion was accepted by the provider but failed transparent viewer continuity with a 5.997-second stall and 0.386-second playhead rollback. Retain it as emergency redundancy, not the ordinary recovery path.
