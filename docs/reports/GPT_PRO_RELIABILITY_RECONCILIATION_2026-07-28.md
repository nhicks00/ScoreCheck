...
| Persist YouTube output through camera loss | Partial | `start-court.sh` starts the renderer without awaiting camera media, so the interruption slate can remain encoded. Camera loss does not intentionally complete YouTube. | Keep invariant. Prove it under physical and synthetic source loss after exact-once supervision is deployed. |
| Exact-once Egress recovery | Implemented; live gate open | The schema-3 owner binds request digest, output generation, destination, and exact renderer identity. A host-local supervisor shares the lifecycle lock, requires repeated absence, rejects owner/active-set ambiguity, verifies the event-local renderer, fully recycles Egress, and replays only the exact owned request with a bounded budget. | Run isolated handler, container, and host-reboot recovery gates before camera admission. |
...
| Final 1080p output conformance | Implemented tool; live evidence open | The conformance gate records an actual local output and uses pinned `ffprobe` to assert H.264 High, 1920x1080, yuv420p progressive Rec.709, selected frame rate/bitrate, two-second-or-shorter keyframes, AAC stereo 128 kbps at 48 kHz, and continuity. | Run it for each output generation and reconcile with YouTube provider/viewer evidence. The next full gate is 1080p30 only. |
| AVKANS SRT/H.264 | Candidate accepted; endurance open | This removes the AVKANS HEVC decoder/normalizer dependency and is compatible with the selected HLS program path. | Use SRT/H.264 for Cameras 3-8 at the next physical gate, subject to exact source admission and endurance. |
...
| H.264 browser-safe admission | Partial | Source admission records codec/profile/fps/dimensions and normalization mode. | Require per-camera identity, firmware, transport, pixel format, progressive scan, exact allowed frame rate, GOP, timestamp monotonicity, bounded bitrate, audio continuity, and duplicate-publisher rejection. Normalize only profiles that actually require it. |
...
| Wi-Fi, RF, power, and thermal evidence | Missing/partial | Router/Speedify state and host resources are recorded. Camera/AP RF, power, and thermal state are not complete. | Add available telemetry without delaying the synthetic release work. Do not prescribe HE80 or another radio width without an on-site survey. |
...
### Phase 1: Synthetic P0 hardening
...
6. Validate actual output conformance and persistent interruption-slate behavior against synthetic sources and unlisted destinations.
...
Nathan will be asked to turn on the router and only the required cameras after Phases 1-2 pass. The initial gate will use one Mevo and one AVKANS camera, 1080p30, SRT, with AVKANS H.264 and a controlled Mevo H.264-versus-HEVC comparison. It will measure delivered bitrate, packet loss/recovery, source continuity, normalizer load, HLS continuity, audio quality, YouTube playback, and Speedify link transitions without changing settings during an accepted measurement window.
...
No router or camera action is required during Phase 1. Nathan should keep the physical equipment off until the synthetic and infrastructure gates are ready. The next request will name the exact cameras, profiles, and expected duration rather than asking for the full venue setup prematurely.
