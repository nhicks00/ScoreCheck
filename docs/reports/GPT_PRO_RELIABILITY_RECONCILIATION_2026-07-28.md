...
| External viewer proof | Implemented; physical gate open | `youtube-viewer-probe.mjs` opens the public YouTube player outside the event media path, bounds browser startup and shutdown, compares frame hashes and luma, requires advancing playback and decoded audio, detects provider sign-in challenges, and fails closed on frozen, black, stalled, silent, or undersized playback. Production soak evidence requires one matching successful observation per active camera, while the continuity mode samples one viewer across ordered sender transitions. | Run it against every unlisted 1080p30 output during the physical eight-camera gate. Provider health remains supporting evidence rather than viewer proof. |
| Durable incident delivery and Pushover receipts | Implemented; dependency-loss gate open | `LocalIncidentOutbox` preserves incident events and notification attempts when Supabase is unavailable. Replay reconciles durable episodes without duplicating notifications. Emergency Pushover receipts are stored and cancelled after actual recovery or expectation cessation; expectation cessation suppresses a normal recovery message. | Exercise a bounded Supabase-loss interval during the physical gate and verify one opening notification, receipt cancellation, durable replay, and no false recovery page. |
