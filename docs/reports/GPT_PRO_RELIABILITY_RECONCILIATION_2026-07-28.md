...
| Exact-once Egress recovery | Timing and ownership passed; final normal-latency visual gate open | The July 29 Camera 5 low-latency viewer stalled for 24.001 seconds. A fresh Camera 6 normal-latency run reduced that to 11.750 seconds. Repeating after changing only the supervisor from five seconds/three missing observations to two seconds/two observations reduced absence confirmation to 2.262 seconds and the external-viewer stall to 1.499 seconds. A later clearly nonblack Camera 5 repeat on its older low-latency destination recovered the exact owner in 12.771 seconds with no duplicate, but still stalled the viewer for 18.501 seconds. This confirms that the supervisor timing works and low-latency destination buffering remains insufficient. The maintenance generation rotation also exposed a stop-drain race; the stop path now retains ownership and intent until LiveKit reports an empty active set. | Keep normal latency and the measured two-second/two-observation timing. Deploy the stop-drain fix, repeat on a clearly nonblack normal-latency destination, then proceed to handler, container, and host-reboot recovery. |
