...
| Exact-once Egress recovery | Supervisor passed; viewer continuity failed under low-latency YouTube | The July 29 Camera 5 physical gate removed the exact owned Egress. The supervisor confirmed absence, replaced the worker container, preserved request generation and digest, and converged to one replacement in 20.705 seconds with no publisher overlap. The external YouTube viewer nevertheless stalled for 24.001 seconds because the broadcast was still configured for low latency. | Hard-cut new broadcasts to normal latency and repeat the same gate before moving to handler, container, and host-reboot recovery. |
...
1. Repeat the passed exact-owner supervisor gate against a fresh normal-latency unlisted broadcast and require the external viewer to remain playback-ready.
