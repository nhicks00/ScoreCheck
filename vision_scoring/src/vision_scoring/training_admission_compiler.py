...
def _normalized_examples_and_references(
    example_manifests: tuple[TrainingExampleManifestV3, ...],
) -> tuple[
    ...
    normalized_rows: list[tuple[TrainingExampleManifestV3, str]] = []
    for example in example_manifests:
        if type(example) is not TrainingExampleManifestV3:
            _fail(
        ...
        try:
            normalized = TrainingExampleManifestV3.from_json_bytes(
                example.to_json_bytes()
            )
        except (AttributeError, TypeError, ValueError) as exc:
            raise TrainingAdmissionCompilerError(
                "TRAIN_COMPILER_INPUT",
                "an example manifest is not exact canonical V3",
            ) from exc
...
def _compile_epoch_selection_v1(
    *,
    ...
    example_rows_by_stratum: dict[
        PrimarySamplingStratumV1,
        tuple[tuple[TrainingExampleManifestV3, str], ...],
    ],
    ...
    ranked_candidates_by_draw: list[
        tuple[tuple[str, str, TrainingExampleManifestV3], ...]
    ] = []
    ...
    selected: list[tuple[str, str, TrainingExampleManifestV3]] = []
...
def compile_training_sampling_schedule_v1(
    *,
    ...
    if not 2 <= len(example_manifests) <= MAX_TRAINING_EXAMPLES:
        _fail("TRAIN_COMPILER_INPUT", "example_manifests exceeds its fixed bound")
    if any(
        type(example) is not TrainingExampleManifestV3
        for example in example_manifests
    ):
