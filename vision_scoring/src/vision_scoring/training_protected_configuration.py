...
DECODER_RUNTIME_PINS_DOMAIN = (
    "multicourt-vision-scoring:decoder-runtime-pins:v1"
)
...
PROTECTED_TRAINING_CONFIGURATION_GENERATION_DOMAIN = (
    "multicourt-vision-scoring:protected-training-configuration-generation:v1"
)
...
def _require_schema_version(value: object, *, label: str) -> None:
    if (
        ...
        or value != TRAINING_PROTECTED_CONFIGURATION_SCHEMA_VERSION
    ):
        raise ValueError(
            f"{label} schema_version must be exactly "
            f"{TRAINING_PROTECTED_CONFIGURATION_SCHEMA_VERSION!r}"
        )
...
@dataclass(frozen=True, slots=True)
class ProtectedTrainingConfigurationGenerationV1(_CanonicalContract):
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.requested_truth_policy) is not AnnotationMinimumTruthPolicy:
            raise ValueError(
                "requested_truth_policy must be an exact "
