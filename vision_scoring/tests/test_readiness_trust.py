...
class StrictReadinessTrustLoaderTests(unittest.TestCase):
    ...
    def test_protected_file_errors_propagate_unchanged_with_schema_caps(self) -> None:
        cases = (
            ...
            (load_dataset_trust_store, 32 * 1024 * 1024, "dataset trust store"),
            (
                load_readiness_verification_policy,
                4 * 1024,
        ...
        for loader, maximum_bytes, label in cases:
            ...
            with self.subTest(loader=loader.__name__), patch(
                "vision_scoring.readiness_trust.read_protected_file_bytes",
                side_effect=error,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for index, (loader, canonical, parser_name) in enumerate(cases):
                ...
                with self.subTest(loader=loader.__name__, case="wrong-type"), patch(
                    f"vision_scoring.readiness_trust.{parser_name}",
                    return_value=object(),
    ...
    def test_loader_preserves_semantic_parser_errors(self) -> None:
        payload = _attestation().to_canonical_dict()
        payload["dataset_id"] = 7
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "invalid-attestation.json"
            path.write_bytes(canonical_json_bytes(payload))
            with self.assertRaisesRegex(
                ValueError,
                "dataset_id must be an ASCII stable ID",
            ):
                load_dataset_manifest_attestation(path)
