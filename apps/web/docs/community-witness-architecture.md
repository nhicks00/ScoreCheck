...
- Public watch-and-score currently uses the court's YouTube broadcast as an advisory transition path. YouTube is not the target scoring transport because its delay is too large for rally-timed evidence. The hard-cut target is authenticated `courtN_preview` WHEP through capacity-qualified read replicas; the existing origin Droplet is not a public fan-out tier.
- While the YouTube transition path remains, focus mode gives the player and score controls separate regions. The provider-neutral layout always contains the complete 16:9 frame, and phone landscape places scoring below the video with a reliable `Score controls` affordance. The owned player may later support true scoring overlays without changing the geometry contract.
...
| Media security | A tempting direct-origin preview path would expose reusable MediaMTX credentials and unbounded fan-out to every participant. | The transition DTO contains only a YouTube video id. The owned-player hard cut requires assignment-scoped bearer authorization, exact `courtN_preview` path scope, read replicas, and removal of anonymous MediaMTX playback before any low-latency descriptor crosses the community boundary. |
...
- Focus mode: the player and score controls share a fullscreen container. Phone and tablet use a complete 16:9 frame followed by score controls in either orientation; phone landscape maximizes the video in the first viewport and exposes scoring below by thumb scroll or the `Score controls` handle. Only short layouts at least 900 px wide use a side dock. Browsers that reject element fullscreen keep the same experience as a fixed full-window dialog.
...
- Until the owned-player hard cut, community media returns only the configured public YouTube video id. Static MediaMTX usernames/passwords, anonymous WHEP/HLS paths, internal hostnames, ingest keys, and reusable preview URLs are forbidden in public session responses. The target contract is defined in `docs/community-low-latency-playback-plan.md`.
...
| Before owned scoring playback | Deliver assignment-scoped, expiring `courtN_preview` authorization plus capacity-qualified read replicas and TURN/TCP connectivity, then hard-cut every browser media consumer. | YouTube delay is not suitable for rally-timed evidence, while reusable credentials and direct origin fan-out are not a secure or scalable public-distribution architecture. |
