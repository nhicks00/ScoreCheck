...
type ScorerContext = {
  ...
  match: (MatchRecord & { source_type?: "vbl" | "manual" | null; api_url?: string | null }) | null;
...
async function persistScorerMutation(
  ...
  if (apiScoreHasPriority(context.currentScore, context.match)) {
    return NextResponse.json({ error: "VolleyballLife live scoring is controlling the broadcast. Manual scoring will resume if that feed stops." }, { status: 409 });
  }
  const idempotent = await findExistingAction(actionId);
  if (idempotent) return NextResponse.json({ ok: true, idempotent: true, score: idempotent.next_state });
