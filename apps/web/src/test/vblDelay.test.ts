...
describe("VBL overlay delay queue", () => {
  it("queues live VBL snapshots for immediate release", () => {
    const delayed = delayedScoreFromSnapshot("match-1", snapshot, startedAt);
    expect(delayed.visibleAt).toBe("2026-07-03T15:00:00.000Z");

    const queued = queueDelayedVblScore([], null, delayed);
    expect(splitDueDelayedVblScores(queued, "2026-07-03T14:59:59.999Z")).toMatchObject({
      latestDue: null,
      remaining: queued
    });

    const due = splitDueDelayedVblScores(queued, "2026-07-03T15:00:00.000Z");
  ...
  it("does not duplicate a score that is already visible or already pending", () => {
    ...
    expect(twice[0].visibleAt).toBe("2026-07-03T15:00:00.000Z");
  });
  ...
  it("holds a due final while a newer final correction is still pending", () => {
    ...
    expect(shouldHoldDelayedFinalScore(correctedFinal, [], "2026-07-03T15:00:15.999Z")).toBe(true);
    expect(shouldHoldDelayedFinalScore(correctedFinal, [], "2026-07-03T15:00:16.000Z")).toBe(false);
  });
});
