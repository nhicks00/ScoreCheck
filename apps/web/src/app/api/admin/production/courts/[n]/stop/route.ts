...
export async function POST(req: NextRequest, { params }: { params: Promise<{ n: string }> }) {
  ...
  if (result.status >= 200 && result.status < 300) {
    try {
      await setCourtBroadcastMonitoringExpectation(courtNumber, "stop");
    } catch {
      result.body.monitoringWarning = "Broadcast stopped, but monitoring expectations could not be cleared. Open System Monitor immediately.";
    }
  }
