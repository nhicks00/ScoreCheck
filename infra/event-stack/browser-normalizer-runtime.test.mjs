...
import { BrowserNormalizerRuntime, parseNormalizerInspect } from "./browser-normalizer-runtime.mjs";

const paths = { sshKey: "/tmp/key", knownHosts: "/tmp/known-hosts" };
const assignment = { sourceCodec: "H265", sourceProfile: "STANDARD_1080P30", frameRateMode: "30/1", mediamtxPrivateHost: "10.20.0.3" };

test("starts and verifies a camera's isolated browser normalizer", async () => {
  const commands = [];
  let started = false;
  const runtime = new BrowserNormalizerRuntime({
    ...
    runner: async (_command, args) => {
      ...
      if (remote.includes("start-normalizer.sh")) {
        started = true;
        return { code: 0, stdout: "Camera 2 browser normalizer is healthy.\n", stderr: "" };
      }
  ...
  assert.equal(result.sourceCodec, assignment.sourceCodec);
...
test("adopts an H264 browser normalizer when direct admission is unsafe", async () => {
  const fixture = inspectFixture(3, "H264");
  const runtime = new BrowserNormalizerRuntime({
    ...paths,
    runner: async () => ({ code: 0, stdout: `${JSON.stringify(fixture)}\n`, stderr: "" }),
    sleep: async () => {}
  });
  const result = await runtime.ensure({ host: "198.51.100.3", court: 3, required: true, ...assignment, sourceCodec: "H264" });
...
test("fails closed when an adopted normalizer does not match the event profile, frame rate, or private ingest", async () => {
  ...
  await assert.rejects(
    () => parseAndValidate(inspectFixture(5), 5, { sourceCodec: "VP9" }),
    /source codec/u
  );
});
...
function inspectFixture(court, sourceCodec = assignment.sourceCodec) {
  return {
    Id: "a".repeat(64),
    ...
    Config: { Env: [
      ...
      `CAMERA_SOURCE_CODEC=${sourceCodec}`,
