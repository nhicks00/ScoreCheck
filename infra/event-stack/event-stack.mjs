...
async function main() {
  ...
  const deployer = needsDeployment ? new LocalStackDeployer({
    ...
    ingestTlsStateDirectory: requiredOption(options.ingestTlsState, "--ingest-tls-state"),
...
function usage() {
  ...
  node infra/event-stack/event-stack.mjs up --manifest FILE --state FILE --anchors FILE --secrets DIR --ssh-key FILE --known-hosts FILE --commentary-tls-state DIR --ingest-tls-state DIR --observability-tls-state DIR --credentials-env FILE --attestation FILE
