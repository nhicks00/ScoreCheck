...
export class EgressRuntime {
  ...
  async listActive(host) {
    const result = await this.#remote(host, "cd /opt/compositor && ./list-egress.sh --active --json");
  ...
  async preflight(host) {
    ...
    const result = await this.#remote(host, "test \"$(docker inspect bvm-egress --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}')\" = healthy && curl -fsS http://127.0.0.1:9090/metrics >/dev/null");
