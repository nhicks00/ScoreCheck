...
rsync -a --delete -e "$rsync_shell" \
  ...
  "$SCRIPT_DIR/fault-owned-egress.sh" \
...
if [[ -f docker-compose.yml && -f .env ]]; then
  ...
  for optional in egress-supervisor.sh fault-owned-egress.sh recycle-egress-worker.sh program-branch-warmer.sh normalize-camera.sh scorecheck-ffmpeg-runner.sh rebind-ingest.sh start-normalizer.sh stop-normalizer.sh; do
...
for file in headless_shell lib.sh list-egress.sh egress-supervisor.sh fault-owned-egress.sh recycle-egress-worker.sh program-branch-warmer.sh normalize-camera.sh scorecheck-ffmpeg-runner.sh qualify-output.sh rebind-ingest.sh start-court.sh start-normalizer.sh stop-normalizer.sh stop-court.sh; do
