...
# Preserve the shared source clock. Numbering surviving video frames and audio
# samples independently turns packet loss into cumulative A/V drift.
exec "$runner" "court${court}_normalizer" -- \
  ...
  -map 0:v:0 -map 0:a:0 \
  -vf "fps=fps=${fps}:start_time=0:round=near:eof_action=pass,format=yuv420p,setfield=prog" -fps_mode passthrough \
