...
[[ "${CAMERA_NORMALIZER_ENABLED:-}" == "true" ]] || { echo "error: this compositor is not assigned a browser normalizer" >&2; exit 64; }
...
rm -f "$progress"
docker compose --profile browser-normalizer up -d --no-deps normalizer
...
docker compose --profile browser-normalizer ps normalizer >&2
docker compose --profile browser-normalizer logs --tail=120 normalizer >&2 || true
echo "error: Camera ${CAMERA_NUMBER} browser normalizer did not become healthy" >&2
exit 1
